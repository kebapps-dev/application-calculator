let genericRotaryProductData = [];
let genericRotaryProductData2 = [];
...
function findClosestGenericRotaryMotor() {
  ...
  const rmsresults = calculateContinuousMotorRating(
    {
        inertia : parseFloat(document.getElementById("genericMomentOfInertia").value), // kg·m²
        targetRPM : parseFloat(document.getElementById("genericRequiredSpeed").value), // RPM
        ...
        frictionTorque : parseFloat(document.getElementById("genericFrictionTorque").value), // Nm
...
//   }
  ...
  //if (closestProduct) {
    resultsDiv.innerHTML = `
      <p><strong>
        Required Motor Power: ${rmsresults.contPowerWithMargin}, ${rmsresults.contPowerHPWithMargin} <br>
        Accel Torque: ${rmsresults.accelTorque} <br>
        RMS Torque: ${rmsresults.rmsTorque} <br>
      </strong></p>`;
